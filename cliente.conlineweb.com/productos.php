...
$clientePageTitle = 'Mis productos';
include('menu.php');
...
<head>
    ...
    <title><?= htmlspecialchars(cliente_document_title('Mis productos'), ENT_QUOTES, 'UTF-8') ?></title>
    <?= cliente_favicon_markup() ?>
</head>
 
  <style>
/* Variante escritorio: se oculta en móvil. Complementaria de .ocultarpc (>=601px). */
...
/* Acotado a los contenedores de la página: sin prefijo, estas reglas
   afectaban a cualquier tabla del documento, incluidas menú y pie. */
.ocultarcel td, .ocultarpc td,
.ocultarcel th, .ocultarpc th {
   text-align: left;
   width: 50%;
...
.ocultarcel table, .ocultarpc table {
...
<?php 
 ...
 <style>
/* Variante móvil: se oculta en escritorio. El corte es 601px para que no
   se solape con .ocultarcel y siempre haya exactamente una visible. */

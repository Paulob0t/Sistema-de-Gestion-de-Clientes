...
if (isset($_POST['id']) && (isset($_POST['eliminar']) || $permanente)) {
    ...
    if ($eliminar === 1 || $permanente) {
        $sqlValidacion = "SELECT COUNT(*) AS total FROM pagos WHERE tipo_servicio = 1 AND id_servicio = ? AND estatus = 0 AND Registro = 0";
        ...
        if ($stmtValidacion) {
            ...
            if (!empty($row['total']) && intval($row['total']) > 0) {
                $response['message'] = 'No se puede eliminar: el servicio de hosting tiene pagos pendientes asociados.';
    ...
    if ($permanente) {
        if ($clienteId > 0) {
            $sql = "DELETE FROM hosting WHERE id_orden = ? AND cliente_id = ?";
            $stmt = $db->prepare($sql);
            if ($stmt) {
                $stmt->bind_param('ii', $id, $clienteId);
            }
        } else {
            $sql = "DELETE FROM hosting WHERE id_orden = ?";
            $stmt = $db->prepare($sql);
            if ($stmt) {
        ...
        if ($stmt) {
            if ($stmt->execute() && $stmt->affected_rows > 0) {
                ...
                $response['message'] = 'Hosting eliminado permanentemente de la base de datos';
            } else {
                $response['message'] = 'No se encontró el hosting o no se pudo eliminar';
            }
            $stmt->close();
        } else {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                $response['success'] = true;
                $response['message'] = $eliminar === 1 ? 'Hosting eliminado correctamente' : 'Hosting restaurado correctamente';
            } else {
